...
- `src/data/references.ts` (typed via `Reference` in `src/types/index.ts`) holds CV referees — name, email, and optional mobile/role/organization/LinkedIn/description. It's rendered only inside `CvAtsDynamic` and `CvStyledDynamic` (as the last section, after Leadership & Comm.), never on the live site. Listing full referee contact details is somewhat old-school; modern CVs often omit the section entirely or use "References available upon request" — may switch to that convention later, so don't be surprised if this section changes shape.
